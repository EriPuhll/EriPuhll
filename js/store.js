...
function subjectPace(s, now = Date.now()) {
  ...
  const credit = Number(s.creditMinutes) || 0; // horas que se dan por hechas (antes de usar la app)
  const done = studiedMinutes(s.id) + credit;
  ...
  const goalAtStart = Math.max(0, goal - credit - studiedMinutes(s.id, 0, firstWeek));
  ...
  const leftAtWeek = Math.max(0, goal - credit - studiedMinutes(s.id, 0, thisWeek));
...
// Única vez: las horas que tocaban antes del lunes 21/9/2026 se dan por hechas
// y el ritmo semanal arranca esa semana, sin deuda.
const PAST_CREDIT_UNTIL = '2026-09-21';
function applyPastWeeksCredit() {
  const set = Store.data.settings;
  if (set.pastCreditApplied) return;
  const cut = parseDate(PAST_CREDIT_UNTIL).getTime();
  if (Date.now() < cut) return;
  Store.data.subjects.forEach((s) => {
    const { start, end } = subjectPeriod(s);
    const frac = clamp((cut - start.getTime()) / Math.max(1, end.getTime() - start.getTime()), 0, 1);
    const owed = goalMinutes(s) * frac - studiedMinutes(s.id, 0, cut);
    s.creditMinutes = Math.max(0, Math.round(owed));
  });
  if (!set.trackingStart || set.trackingStart < PAST_CREDIT_UNTIL) set.trackingStart = PAST_CREDIT_UNTIL;
  set.pastCreditApplied = true;
  Store.save();
}
