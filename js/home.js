...
function renderHome() {
  ...
  $('#view').innerHTML = `
    <section class="page">
      ...
      ${alerts.length ? `<section class="card"><h2>Para tener en cuenta</h2><div class="alert-list">${alerts.map((x) => `<div class="alert ${x.cls}"><span>${x.html}</span></div>`).join('')}</div></section>` : ''}
